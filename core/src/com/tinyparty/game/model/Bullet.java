...
public class Bullet extends Entity {
	...
	public Bullet(TinyParty game, Vector2 position, Vector2 direction, float angle, boolean sourceOfFire, int size, float speed, float damage, float distance) {
		...
		if(sourceOfFire) {
			this.body = PhysicManager.createBox(position.x, position.y, 2f, 1f, angle, Constants.BULLET_MOVE_CATEGORY, Constants.BULLET_MOVE_MASK, false, true, this, game.getGameScreen().getWorld());
		}
		else {
			this.body = PhysicManager.createBox(position.x, position.y, 2f, 1f, angle, Constants.OTHER_BULLET_CATEGORY, Constants.OTHER_BULLET_MOVE_MASK, false, true, this, game.getGameScreen().getWorld());
		}
	...
	public void render(Batch batch, AssetManager assetManager) {
		batch.draw(assetManager.get(Asset.STANDARD_BULLET.filename, Texture.class), body.getPosition().x - 4f/2f, body.getPosition().y - 4f/2f - 3f, 4f/2f, 4f/2f,
				4f, 4f, size, size, 0f, 0, 0, 4, 4, false, false);
	}
